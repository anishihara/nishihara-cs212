    ...
    for word1 in words:
        for word2 in words:
            if word1 != word2:
                ...
                for p in word2:
                    part_word += p
                    if part_word in word1:
                        part1 = word1.partition(part_word)
                        part2 = word2.partition(part_word)
                        start = part1[0]
                        mid = part_word
                        end = part2[2]
                        if start != '' and mid != '' and end != '':
                            portmanteau = start+mid+end
                            ideal_start = float(len(portmanteau))/4.
                            ideal_mid = float(len(portmanteau))/2.
                            ideal_end = float(len(portmanteau))/4.
                            score = float(len(portmanteau)) - abs(len(start)-ideal_start) - abs(len(mid)-ideal_mid) - abs(len(end)-ideal_end)
                            if score > old_score:
                                old_score = score
                                result = portmanteau
                            portmanteau_list[portmanteau] = score
    if len(portmanteau_list) > 0:
        return result
    else:
        return None
