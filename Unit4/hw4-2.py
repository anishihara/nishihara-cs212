...
def more_pour_problem(capacities, goal, start=None):
    ...
    def psuccessors(state):
        """Given a state, return a dictionary {state:action} where state is a tuple
        of of numbers representing the current level in each glass.
        """
        #fill
        result = {}
        for i in range(len(capacities)):
            if state[i] < capacities[i]:
                newstate = [a for a in state]
                newstate[i] = capacities[i]
                newstate = tuple(newstate)
                action = ('fill',i)
                result[newstate] = action
        #empty
        for i in range(len(capacities)):
            if state[i] != 0:
                newstate = [a for a in state]
        ...
        for i in range(len(capacities)):
            for j in range(len(capacities)):
                if state[j] < capacities[j] and i != j:
                    newstate = [a for a in state]
                    if state[i]+state[j] <= capacities[j]:
                        difference = state[i]
                    else:
                        difference = capacities[j]-state[j]
                    newstate[i]=state[i] - difference
                    newstate[j]=state[j] + difference
                    newstate = tuple(newstate)
                    result[newstate] = ('pour',i,j)

        return result
    if start is None:
        start = tuple([0 for a in range(len(capacities))])
    pgoal = lambda x: goal in x
    return shortest_path_search(start,psuccessors,pgoal)
